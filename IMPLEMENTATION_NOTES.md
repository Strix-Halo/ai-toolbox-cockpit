...
DS4 server containers ran without the host RDMA devices, so RoCE transport had nothing to bind to inside the container. InfiniBand passthrough now lives in `ai_toolbox_cockpit/runtime/rdma.py` as shared Podman/Docker logic: `host_rdma_device_nodes()` lists the nodes under `/dev/infiniband` and `container_rdma_args(engine, rdma_path)` returns `--device <dir> --group-add rdma --ulimit memlock=-1` for Podman or one `--device <node>` per node plus the memlock ulimit for Docker. Detection is presence-based — no `/dev/infiniband`, no flags — so non-InfiniBand hosts are byte-identical. `build_server_cmd` merges those args with `extend_missing_option_pairs()` before `upgrade_groups_for_podman()`, so on Podman the named groups still collapse to a single `--group-add keep-groups` and the net additions are the device and the memlock ulimit. This applies to every DwarfStar toolbox profile and both engines, in standalone, coordinator, and worker mode. `llama_cpp.get_server_rdma_args()` keeps its Strix Halo platform gate and now delegates to the same helper instead of duplicating the engine branches. Halogen stays excluded by design: its isolation test rejects profile-supplied `--device /dev/infiniband`.

Interactive Toolbx environments need no equivalent change and never did. Toolbx creates its own containers with `--privileged`, `--ulimit host` and `--volume /dev:/dev:rslave`, so the host `/dev`, InfiniBand nodes included, is already inside the environment; `toolbox create` exposes no engine-argument passthrough because it does not need one. Only the Distrobox path passes the RDMA devices explicitly. Verified against the installed Toolbx 0.3 binary and its `src/cmd/create.go`.

## 2026-09-19 — Tensor-parallel worker runs the `ds4` CLI without HTTP options

`ds4` is the interactive/one-shot CLI, not a daemon, so it rejects `--host` and `--port` with `ds4: unknown option` and exits 2. The tensor-parallel worker now omits both: `config.is_tensor_parallel_cli_worker(model_path, role, tensor_parallel)` is the single predicate for "this role runs the plain CLI", `resolve_server_binary()` uses it for the binary, and the command builder skips `--host`/`--port` whenever it is true. The worker preview now reads `ds4 -m /models/<model> --ctx <ctx> [--vision /models/<encoder>] --tensor-parallel --role worker --coordinator <ip> <port> --transport <tcp|rdma>`. The Server panel disables the host and port fields while a tensor-parallel worker is selected and says why in `#ds4-tp-note`; unchecking tensor parallelism or switching role restores them. Coordinators, standalone launches, and every other family still receive `--host 0.0.0.0 --port <port>` from `ds4-server`.

## 2026-09-19 — DeepSeek V4.1 Flash tensor-parallel workers launch `ds4`

The DeepSeek V4.1 Flash tensor-parallel worker is served by the `ds4` CLI, not the `ds4-server` binary declared by the DS4 toolbox profiles. `ai_toolbox_cockpit/backends/ds4/config.py` now owns that policy in one explicit mapping, `TENSOR_PARALLEL_WORKER_BINARIES = {"deepseek-v4.1-flash": "ds4"}`, exposed through `resolve_server_binary(model_path, role, tensor_parallel, default_binary)`. The command builder resolves the binary through that helper, so the override applies only when tensor parallelism is active and the role is Worker. Coordinators, standalone launches, non-tensor-parallel workers, and every other family keep the toolbox's `server_binary` (default `ds4-server`). The toolbox JSON is unchanged, so no other profile is affected. The tensor-parallel zone shows the substitution in `#ds4-tp-note` while a Worker with tensor parallelism is selected, and the start confirmation still prints the full command before launch. Command and headless UI tests cover the worker override, the unchanged coordinator/standalone/other-family binaries, and the note lifecycle.
