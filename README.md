...
[Halogen Flash](https://github.com/peonist-ai/halogen-flash-server) is a third-party closed-source project and container for Qwen3.8-Flash-Next on gfx1151. **Use at your own risk:** its implementation is harder to inspect and audit than the open-source backends.

Cockpit creates the Halogen server container with `--network=none`, giving it no direct Internet, LAN, or host-network access. It mounts only the selected checkpoint, overlay, tokenizer files, and optional vision weights read-only, plus the GPU devices required by the runtime profile. It does not mount your home, the entire model directory, or engine sockets, and does not pass your Hugging Face token to the server. These restrictions reduce risk but do not remove it: the container still shares the host kernel and GPU driver, retains the profile's host IPC and unconfined seccomp settings, and can return anything it can read through API responses.

The API remains reachable through a Cockpit host listener, which forwards each incoming connection through `podman exec -i` or `docker exec -i` to the fixed API port on container loopback. No container ports are published and the relay provides no outbound route. It uses the image's `python3`, preserves streaming responses, and closes when serving stops. Host defaults to `127.0.0.1`; selecting a non-loopback address exposes the API on that interface without adding authentication. Run Cockpit on the GPU host; the relay binds where Cockpit runs.

Cockpit follows `ghcr.io/peonist-ai/halogen-flash-server:latest` and uses `--pull=always` with Podman and Docker to fetch the current image before each server launch. The host engine still needs registry access; a failed pull stops the launch. Model downloads also happen separately on the host. Model bundles remain revision-pinned; GPU operation and effective isolation remain experimental until validated on the remote GPU host.
...
4. In **Server Mode → Halogen Flash**, select the matching precision bundle and start it. Required file sizes are checked before launch, including the selected overlay and tokenizer. This detects missing/truncated files, but is not a checksum verification. Individual bundle files are mounted read-only and the image's own entrypoint starts both engine and API. The host API relay defaults to `127.0.0.1:8731`.
